...
?>
  ...
  <div id="page-wrapper">
    ...
    <!-- Page Heading -->
      <div class="row">
        <div class="col-lg-12">
          <h1 class="page-header">디자인 투표 참여자 수 PC / Mobile</h1>
        </div>
      </div>
      <!-- /.row -->
      <div class="row">
          ...
          <div class="table-responsive">
            ...
            <div id="daily_applicant_count_div1" style="display:block">
              <table class="table table-hover">
                ...
                <tbody>
<?php
	$daily_date_query	= "SELECT design_regdate FROM ".$_gl['design_info_table']." WHERE 1 Group by substr(design_regdate,1,10) ORDER BY design_regdate DESC";
	$date_res			= mysqli_query($my_db, $daily_date_query);
	while($date_daily_data = mysqli_fetch_array($date_res))
	{
		$daily_date		= substr($date_daily_data['design_regdate'],0,10);
		$media_query	= "SELECT design_media, COUNT( design_media ) media_cnt FROM ".$_gl['design_info_table']." WHERE 1 AND design_regdate LIKE  '%".$daily_date."%' GROUP BY design_media";
		$media_res		= mysqli_query($my_db, $media_query);

		$unique_query	= "SELECT * FROM ".$_gl['design_info_table']." WHERE 1 AND design_regdate LIKE  '%".$daily_date."%' GROUP BY design_ipaddr";
		...
		while ($media_daily_data = mysqli_fetch_array($media_res))
		{
			...
			$pc_query		= "SELECT * FROM ".$_gl['design_info_table']." WHERE 1 AND design_regdate LIKE  '%".$daily_date."%' AND design_media='".$media_daily_data['design_media']."' AND design_gubun='PC'";
			$pc_count		= mysqli_num_rows(mysqli_query($my_db, $pc_query));
			$mobile_query	= "SELECT * FROM ".$_gl['design_info_table']." WHERE 1 AND design_regdate LIKE  '%".$daily_date."%' AND design_media='".$media_daily_data['design_media']."' AND design_gubun='MOBILE'";
